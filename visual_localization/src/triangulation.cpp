...
namespace VISUAL_MAPPING {
    ...
    //    std::cout<<"kp2 "<<kp2<<std::endl;
        ...
        // 1. Check parallax远处的点视差就是很小呀
        Eigen::Vector3d r21= R * r2;
        const double cosParallaxRays = r1.dot(r21) / (r1.norm() * r21.norm());
        if (cosParallaxRays > 0.9998) {
            //std::cout<<"cosParallaxRays > 0.9998:"<<cosParallaxRays<<std::endl;
            return std::make_pair(-1, 100);
        }
        // double disp = kp1.x() - kp2.x();  // 注意左右相机顺序
        // if (std::fabs(disp) < 1.0) return {-1, 100}; // 小于 1 像素就别三角化了（不稳定）
